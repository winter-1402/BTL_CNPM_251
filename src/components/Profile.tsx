...
import { Mail, Phone, MapPin, Calendar, Edit, Book, Award, Briefcase, GraduationCap, BookOpen, Star, UserCheck, Clock } from 'lucide-react';
...
// Khởi tạo kiểu dữ liệu BaseProfile (chung cho cả hai)
type BaseProfileData = User & {
    phone: string;
    address: string;
    joinDate: string;
    bio: string;
...
type TutorProfileData = BaseProfileData & {
    degree: string;
    university: string;
    experience: string;
    expertise: string[];
...
// ====================================================================
// 🛠️ COMPONENT TÁI SỬ DỤNG: FORM CHỈNH SỬA PROFILE CƠ BẢN
// ====================================================================

interface EditProfileDialogProps {
    initialData: BaseProfileData;
    onUpdate: (updates: Partial<BaseProfileData>) => void;
    title: string;
}

function EditProfileDialog({ initialData, onUpdate, title }: EditProfileDialogProps) {
    const [open, setOpen] = useState(false);
    const [formData, setFormData] = useState<Partial<BaseProfileData>>({
...
function StudentProfile({ user }: { user: User }) {
  const initialData: StudentProfileData = {
    ...
    mssv: '2314567',
    className: 'MT23KHM3',
    lastLogin: '10 giây trước',
    interests: ['Cấu trúc dữ liệu & Giải thuật', 'Kiến trúc máy tính', 'Nguyên lý ngôn ngữ lập trình', 'Hệ cơ sở dữ liệu', 'Công nghệ phần mềm', 'Mạng máy tính'],
    achievements: [
      'Top 10% lớp',
  ...
  const [profileData, setProfileData] = useState<StudentProfileData>(initialData);

  const handleUpdateProfile = (updates: Partial<BaseProfileData>) => {
  ...
  const academicInfo = [
    ...
    { label: 'Tín chỉ tích luỹ', value: '120 tín chỉ' },  
  ];
    ...
    <div className="p-6 space-y-6">
      <div className="flex items-center justify-between">
        ...
        <EditProfileDialog
            ...
            title="Chỉnh sửa Hồ sơ Sinh viên"
        />
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
            ...
            <CardContent className="p-6">
              <div className="flex flex-col items-center text-center">
                ...
                <div className="w-full space-y-3 text-left">
                  {/* THÔNG TIN HỌC TẬP CƠ BẢN (MSSV, KHOA, LỚP) */}
                  <div className='space-y-1'>
                    <div className="flex justify-between items-center text-sm text-gray-600">
                      <span className='font-medium text-gray-700'>MSSV:</span>
                      <span>{profileData.mssv}</span>
                    </div>
                    <div className="flex justify-between items-center text-sm text-gray-600">
                      <span className='font-medium text-gray-700'>Khoa:</span>
                      <span>{profileData.faculty || 'Khoa Khoa học máy tính'}</span>
                    </div>
                    <div className="flex justify-between items-center text-sm text-gray-600">
                  ...
                  {/* THỜI GIAN THAM GIA & ĐĂNG NHẬP */}
                  <div className="text-xs space-y-1 text-gray-600">
                    <div className="flex items-center gap-2">
                        <Calendar className="h-4 w-4 text-gray-500" />
                        <span>Đã tham gia từ: {profileData.joinDate}</span>
                    </div>
                    <div className="flex items-center gap-2">
                        <Clock className="h-4 w-4 text-gray-500" />
                        <span>Lần cuối đăng nhập: {profileData.lastLogin}</span>
                    </div>
                  </div>
        ...
        <div className="lg:col-span-2 space-y-6">
          ...
          {/* LĨNH VỰC QUAN TÂM (Đã chuyển về dạng Badge và đặt dưới Chi tiết học tập) */}
          <Card className="border border-gray-900">
            <CardHeader>
              <CardTitle className="text-lg">Các môn quan tâm</CardTitle>
            </CardHeader>
            <CardContent>
              {/* Chuyển về dạng Badge ngang */}
          ...
          {/* Achievements (ĐÃ BỎ KHỐI NÀY) */}

          {/* Recent Activity (Viền đen) */}
          <Card className="border border-gray-900">
            <CardHeader>
...
function TutorProfile({ user }: { user: User }) {
    const initialTutorData: TutorProfileData = {
        ...user,
    ...
    const [tutorData, setTutorData] = useState<TutorProfileData>(initialTutorData);

    const handleUpdateProfile = (updates: Partial<BaseProfileData>) => {
        setTutorData((prev: TutorProfileData) => ({ ...prev, ...updates }));
    };
            ...
            <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
                ...
                <div className="lg:col-span-1">
                    ...
                    <Card className="mt-6">
                        ...
                        <CardContent>
                            <div className="flex flex-wrap gap-2">
                                {tutorData.expertise.map((skill: string, index: number) => (
                ...
                <div className="lg:col-span-2 space-y-6">
                    ...
                    {/* KHỐI THỐNG KÊ (ĐÃ ĐƯỢC CHUYỂN SANG ĐÂY) */}
                    <Card> 
                        <CardHeader>
                                ...
                                <CardContent>
                                    <div className="grid grid-cols-2 gap-4">
                                        <div className="p-4 bg-gray-50 rounded-lg">
                                            ...
                                            <p className="text-gray-900">{tutorData.faculty || 'Khoa KHMT'}</p>
                                        </div>
                                        <div className="p-4 bg-gray-50 rounded-lg">
...
// ====================================================================
// 🧠 COMPONENT CHÍNH (PHÂN LUỒNG)
// ====================================================================

export function Profile({ user }: ProfileProps) {
    // Phân luồng logic dựa trên vai trò của người dùng
    if (user.role === 'tutor') {
        return <TutorProfile user={user} />;
    }
    
    // Mặc định hoặc nếu user.role === 'student'
